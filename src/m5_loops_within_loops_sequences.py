...
def run_test_integers():
    ...
    # Test 2:
    expected = [12,10,3,3,3,234,0]
    answer = integers([(12,10,3),('peaches','a',3),[3,234,0],[[4],'a']])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
    ...
    for k in range(len(sequence_of_sequences)):
        for j in range(len(sequence_of_sequences[k])):
            if type(sequence_of_sequences[k][j]) == int:
                answer = answer + [sequence_of_sequences[k][j]]
    return answer
...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'SAUSAGEOOPSOOPSPSFFFFOPOO'
    answer = big_letters([(3, 1, 4),  # not a string
                          'SAUSAGE',  # OTSSS
                          (10, 'little sausaGe', 10),  # not a string
                          [],  # not a string
                          ['oops'],  # not a string
                          'oops',  #
                          ['OOPS'],  # not a string
